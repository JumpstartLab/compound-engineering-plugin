...
status: active
date: 2026-05-10
revised: 2026-05-10 (post-Unit-0-spike: --session flag fix, hostname-only allowlist, daemon spawn-time caveat, CLI syntax pre-teach)
...
- `plugins/compound-engineering/skills/agent-browser/SKILL.md` — verified features used by this plan: `--session <name>` for per-instance state and parallel isolation (the daemon spawns a separate process per unique `--session`); `agent-browser state clear <name>`; `AGENT_BROWSER_ALLOWED_DOMAINS`, `AGENT_BROWSER_CONTENT_BOUNDARIES=1`, `AGENT_BROWSER_ACTION_POLICY`, `AGENT_BROWSER_ENCRYPTION_KEY`. Both `Bash(npx agent-browser:*)` and `Bash(agent-browser:*)` are in its `allowed-tools` frontmatter. **The Unit 0 spike empirically verifies that concurrent `--session` invocations from parallel subagents do not contend on shared state** — if they do, the design rolls to scout-plus-critic.
...
- `docs/solutions/integrations/agent-browser-chrome-authentication-patterns.md` — `--session` is the reliable mechanism for persistent auth state with agent-browser.
...
- **Unit 0 spike runs FIRST.** Before any production-shipping code, validate sonnet × agent-browser × `--session` isolation on one persona × one feature surface. This converts the "smoke test as gate" claim from theory to practice and protects Units 1-6 against being a sunk cost if the architecture fails. If the spike fails, roll to scout-plus-critic before writing the v1 implementation.
...
- **`agent-browser` flag standardization: `--session <name>` is used consistently for everything** — auth-state persistence, parallel isolation, close, and state clear. **Confirmed empirically by Unit 0 spike (see `docs/spikes/2026-05-10-user-scenarios-direct-drive.md`):** `--session <name>` spawns an isolated browser process per name with its own cookies and tabs; two concurrent `--session` values do NOT cross-contaminate. The plan's earlier draft referred to `--session-name`, which is a different flag (named cookie save/restore WITHOUT process isolation) — spike caught the bug and the plan was corrected before any production code shipped. **Daemon spawn-time caveat:** environment variables (`AGENT_BROWSER_ALLOWED_DOMAINS`, `AGENT_BROWSER_CONTENT_BOUNDARIES`, `AGENT_BROWSER_ENCRYPTION_KEY`) are read by the agent-browser daemon at first-invocation spawn time and cached. Subsequent invocations on the same `--session` ignore env-var changes. Operational requirement: `state clear <name>` MUST precede any re-spawn that needs different env values. The cleanup contract (Unit 5) already issues `state clear` on every terminal state, satisfying this.
- **`AGENT_BROWSER_ALLOWED_DOMAINS` is verified-exists and enforced.** Confirmed empirically by Unit 0 spike: off-allowlist navigation is blocked with `✗ Domain '<host>' is not in the allowed domains list` and exit code 1. **Format is hostname only, NOT host:port** — `AGENT_BROWSER_ALLOWED_DOMAINS=localhost:3000` rejects all navigation (it treats `localhost:3000` as a literal hostname, which never matches). Correct form: `AGENT_BROWSER_ALLOWED_DOMAINS=localhost` for KickScout's localhost-only dev setup; comma-separated hostnames (no ports) for multi-host setups. Unit 4 implementation must strip the port from `url:` and `mail_capture_url` before composing the env-var value. `AGENT_BROWSER_CONTENT_BOUNDARIES=1` enabled for nonce-tagged page content. `AGENT_BROWSER_ENCRYPTION_KEY=$(openssl rand -hex 32)` is set per-run (Unit 4) to encrypt session state files at rest as defense-in-depth against unclean-exit-leaves-plaintext-tokens scenarios. Resolves origin Deferred Question #6.
...
- **Cleanup contract uses explicit terminal-state branching.** Four states: `success`, `failure`, `timeout`, `partial`. On every state: `npx agent-browser --session <name> close && npx agent-browser state clear <name>` per persona. Discard the per-run `BROWSER_ENCRYPTION_KEY` from environment after cleanup. `.context/compound-engineering/ce-user-scenarios/<run-id>/` retained by default; cleaned only when caller passes `CE_USER_SCENARIOS_CLEANUP=1` AND state is `success`. Resolves origin Deferred Question #5.
...
- **Whether agent-browser uses per-`--session` daemon instances (giving parallel isolation) or a shared daemon serving multiple session names** — answered by Unit 0 spike. If shared, isolation guarantee for persona failures is incorrect and Unit 4 must document the limitation.
...
```mermaid
flowchart TB
    Z[Unit 0 SPIKE<br/>1 persona, 1 surface, manual<br/>sonnet + npx agent-browser<br/>--session unique<br/>verify: auth, navigate, screenshot,<br/>parallel isolation, allowlist] -->|spike passes| A[/ce:user-scenarios<br/>stage:implementation<br/>url:... auth-config:...]
    ...
    J --> K[Spawn N persona subagents<br/>model: sonnet<br/>each with:<br/>- --session unique to persona+run<br/>- AGENT_BROWSER_ALLOWED_DOMAINS scope<br/>- AGENT_BROWSER_CONTENT_BOUNDARIES=1<br/>- AGENT_BROWSER_ENCRYPTION_KEY<br/>- per-persona email_env identity<br/>- action budget caps 40/20/300<br/>- evidence-output requirement]
    K --> L[Personas drive app in parallel<br/>npx agent-browser --session ...<br/>screenshots to RUN_ID/persona/<br/>narrative cites screenshots<br/>by relative path]
...
- [ ] **Unit 0: Sonnet × agent-browser × `--session` reliability spike**
...
- Concurrency check: run TWO sonnet subagent invocations in parallel with distinct `--session` values (e.g., `spike-dorry` and `spike-chuck`). Verify they don't contend on browser state — neither sees the other's logged-in session.
...
- **CLI syntax pre-teach (from Unit 0 spike findings):** the live-app stage blocks include a "common syntax" note: (a) `click <selector>` accepts `@<ref>` for refs returned by `snapshot` — e.g., `npx agent-browser --session {session_name} click @e5`; (b) Turbo-driven links may not navigate when clicked via `@<ref>` — extract the `href` via `eval` or read the URL from the snapshot, then use `open <url>` instead. Pre-teaching this saves 1-2 invocations per persona that would otherwise be burned on syntax discovery.
...
**Goal:** Wire `--session`, allowlist, content boundaries, encryption key, and action-budget enforcement into Step 4 dispatch.
...
- Step 4 dispatch wrapper exports per-run env vars before spawning persona subagents:
  ...
  - `AGENT_BROWSER_ALLOWED_DOMAINS="<url-hostname>,<mail-capture-hostname-if-any>"` — **hostname only, no port.** Parse the URL with a standard library (e.g., Ruby's `URI` or shell `awk`), extract `host` (which excludes the port). Empirically: `localhost:3000` as a value rejects ALL navigation; bare `localhost` permits it. Spike-confirmed.
  - `AGENT_BROWSER_CONTENT_BOUNDARIES=1`
- **Daemon spawn-time semantics (Unit 0 spike finding):** agent-browser spawns a daemon per `--session` value on first invocation and caches env vars at spawn time. Subsequent invocations on the same `--session` ignore changes to `AGENT_BROWSER_ALLOWED_DOMAINS`. The cleanup contract's `state clear <name>` (Unit 5 Step 7) is what enables the next run with different env values — that's not optional, it's load-bearing. Document this in the SKILL.md text near the env-var exports.
...
- Session name: `SESSION_NAME=ce-user-scenarios-${RUN_ID}-${PERSONA_NAME}` substituted as a literal into template variables. Persona template requires `--session "{session_name}"` on every `npx agent-browser` command.
...
- *Describe block: Unit 4 contract.* SKILL.md Step 4 mentions `AGENT_BROWSER_ALLOWED_DOMAINS`, `AGENT_BROWSER_CONTENT_BOUNDARIES`, `AGENT_BROWSER_ENCRYPTION_KEY`, `--session`, action-budget literals (40, 20, 300); contains the silent-failure detection text; `SESSION_NAME=ce-user-scenarios-${RUN_ID}-${PERSONA_NAME}` literal present; **contains the "hostname only — strip port" note about composing `AGENT_BROWSER_ALLOWED_DOMAINS`**; contains the daemon-spawn-time env-var caveat note (env vars cached at first spawn; `state clear` required to re-read).

- *Describe block: Unit 5 contract.* SKILL.md Step 5 mentions relative-path citation; Step 5 documents screenshot-existence validation + `fabricated-citation` tagging; Step 6 includes an `### Evidence Conflicts` subsection header; Step 7 enumerates all 4 terminal states; contains both `npx agent-browser --session {session_name} close` and `npx agent-browser state clear {session_name}` literally; documents `CE_USER_SCENARIOS_CLEANUP=1` env-var opt-in (and does NOT reference any `cleanup-on-success:` arg).
...
- **Interaction graph:** `/ce:run erin url:... auth-config:...` → Erin orchestrator's `everyday-usability` phase (args contain `$ARGUMENTS`) → `/ce:user-scenarios stage:implementation ... url:... auth-config:...` → 5 parallel persona subagents (model: sonnet) → each invokes `npx agent-browser --session ...` via Bash. Depth: main → 1 (skill) → 2 (persona subagents). agent-browser is a leaf.

...
| `mail_link_recipient_selector` | yes | non-empty string | CSS or accessibility selector that locates the persona's specific email in the mail capture inbox, given the persona's email value. May contain a literal `%EMAIL%` placeholder which the persona resolves to the value of `email_env` before using |
...
Each persona is assigned a distinct `email_env` value at dispatch time (e.g., `KS_BETTY_EMAIL`, `KS_CHUCK_EMAIL`, etc.). Per-persona unique emails are how the skill avoids inbox-ambiguity in the magic-link flow.
...
- URL fields must use `http` or `https` scheme and resolve to a non-private, non-loopback, non-link-local IP — **except** `mail_capture_url`, which is loopback-exempt.
